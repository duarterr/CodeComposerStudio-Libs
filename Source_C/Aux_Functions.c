...
float Aux_Map(float Value, float InMin, float InMax, float OutMin, float OutMax)
{
    // Constrain value
    if (Value >= InMax)
        return OutMax;
...
char* Aux_Strcpy(char* Destination, const char* Source)
{
...
void Aux_StrReverse(char* Begin, char* End)
{
    while (Begin < End)
    {
        char C = *Begin;
...
uint8_t Aux_L2Str(int32_t Number, char* String)
{
    char* Pointer = String;
    ...
    Aux_StrReverse(String + (*String == '-'), Pointer - 1);
...
uint8_t Aux_F2Str(float Number, char* String, uint8_t DecPlaces)
{
    // Variables
    uint8_t Length = 0;
    uint8_t Counter = 0;
    ...
    for (Counter = 0; Counter < DecPlaces; Counter++)
        RoundingFactor *= 0.1;
